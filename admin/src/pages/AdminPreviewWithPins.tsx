...
export default function AdminPreviewWithPins({
  ...
  const [variantId, setVariantId] = useState(initialVariantId || packs?.[0]?.variantId || "na");
  ...
  const recompute = () => {
    ...
    for (const h of highlights) {
      ...
      next.push({ action_id: h.action_id, top, left, w: r.width, h: r.height });
    }
    setPins(next);
  };
              ...
              {highlights.map((h, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{ marginTop: 8, fontSize: 13, opacity: 0.85, lineHeight: 1.6 }}>{h.reason}</div>
                  </button>
                );
              })}
            </div>
