...
const TOOLS = [
    ...
    {
        name: "get_purchases",
        description: "指定期間の購入明細を1件ずつ返します。各購入の売上・商品(items)・天気(weather_label/temp)・地域(都道府県)・クーポンを含むため、「雨の日に売れた商品」「気温20℃以下の売れ筋」「地域別の人気商品」などのクロス分析に使えます。返ってきた明細をその場で集計して回答すること。",
        inputSchema: {
            type: "object",
            properties: {
                site_id: { type: "string", description: "サイトID" },
...
// ── プランチェック ────────────────────────────────────────────────
async function checkMcpPlanEnabled(uid) {
    const db = (0, admin_1.adminDb)();
    ...
    if (sitesSnap.empty)
        return false;
    const workspaceId = String(sitesSnap.docs[0].data().workspaceId || "");
    if (!workspaceId)
        return false;
    ...
    if (billing.access_override_active && billing.access_override_until) {
        if (new Date(billing.access_override_until) > new Date())
            return true;
    }
...
async function executeTool(name, args, uid) {
    ...
    if (name === "get_visitor_journey") {
        ...
        const snap = await db.collection("logs")
            ...
            .get();
        if (snap.empty)
            return `訪問者 ${visitor_id} のログが見つかりません。`;
        const EVENT_ICON = {
    ...
    if (name === "get_page_paths") {
        ...
        for (const d of snap.docs) {
            ...
            if (goal_event && data.event === goal_event)
                visitorGoals.add(vid);
        }
        ...
        if (!targetVisitors.length)
            return `該当する訪問者の経路データがありません。`;
    ...
    if (name === "get_purchases") {
        ...
        const snap = await db.collection("logs")
            ...
            .get();
        if (snap.empty)
            return `期間 ${day_from}〜${day_to} に購入はありません。`;
        ...
        const purchases = rows.map((p) => ({
            ...
            items: Array.isArray(p.items)
                ? p.items.map((it) => ({ title: String(it.title || ""), qty: Number(it.qty) || 0, price: Number(it.price) || 0 }))
                : [],
    ...
    if (name === "get_top_pages") {
        ...
        for (const d of logsSnap.docs) {
            const data = d.data();
            const path = String(data.path || data.url || "(不明)");
