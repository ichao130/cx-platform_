...
const PageAttributionReqSchema = zod_1.z.object({
    site_id: zod_1.z.string().min(1),
    day_from: zod_1.z.string().regex(/^\d{4}-\d{2}-\d{2}$/),
    day_to: zod_1.z.string().regex(/^\d{4}-\d{2}-\d{2}$/),
    // 集計から除外したいパス接頭辞（例: "/cart", "/checkout"）。ラストタッチのノイズ除去用
    exclude_prefixes: zod_1.z.array(zod_1.z.string()).optional(),
    limit: zod_1.z.number().int().positive().max(500).optional(), // 返す上位ページ数
});
// 訪問者タグ設定（ジャーニーの追跡・検索用）
const VisitorTagsSetReqSchema = zod_1.z.object({
    site_id: zod_1.z.string().min(1),
...
function registerV1Routes(app) {
        ...
        try {
            ...
            const body = zod_1.z.object({
                ...
                aiContext: zod_1.z.object({
                    sells: zod_1.z.string().optional(),
                    target: zod_1.z.string().optional(),
                    brandDo: zod_1.z.string().optional(),
                    brandDont: zod_1.z.string().optional(),
                    campaign: zod_1.z.string().optional(),
                }),
            }).parse(req.body);
            await requireWorkspaceAccessBySiteId(req, body.site_id, "sites", ["owner", "admin", "member"]);
            const db = (0, admin_1.adminDb)();
            await db.collection("sites").doc(body.site_id).set({ aiContext: body.aiContext, updatedAt: firestore_1.FieldValue.serverTimestamp() }, { merge: true });
    ...
    app.get("/v1/serve", async (req, res) => {
        try {
            ...
            const abKey = String(req.query.vid ||
                req.query.sid ||
            ...
            for (const doc of scenarioSnap.docs) {
                ...
                if (s?.targeting?.enabled && !isTestMode) {
                    ...
                    if (t.audience.urlRules.length > 0) {
                        const urlOk = t.audience.urlRules.some((rule) => matchStringRule(reqPath, rule) || (!!reqUrl && matchStringRule(reqUrl, rule)));
                        if (!urlOk)
                            continue; // このページはURL条件に合致しない → スキップ
                    }
                }
    ...
    app.post("/v1/log", async (req, res) => {
        try {
            ...
            let attributedScenarioId = body.scenario_id ?? null;
            ...
            if (event === "purchase" && attributedScenarioId) {
                ...
                try {
                    ...
                    const sched = scSnap.exists ? scSnap.data()?.schedule : null;
            ...
            if (body.vid) {
                ...
                if (event === "pageview") {
                    ...
                    if (typeof body.revenue === "number") {
                        vrUpdate.revenue = firestore_1.FieldValue.increment(body.revenue);
                    }
                }
        ...
        try {
            ...
            const firstPurchaseTs = new Map();
            {
                ...
                for (const d of pSnap.docs) {
                    ...
                    if (!v)
                        continue;
                    if (!firstPurchaseTs.has(v))
                        firstPurchaseTs.set(v, String(pd.createdAt || "")); // asc順なので最初=初回
                }
            }
            const pages = new Map();
            const getAgg = (p) => {
            ...
            const legPages = new Map();
            const seenOrders = new Set(); // order_id重複排除（同一注文の多重ログ対策）
                ...
                for (const d of snap.docs) {
                    ...
                    if (data.event === "pageview") {
                        ...
                        for (const p of leg) {
                            ...
                            if (isRepeat)
                                a.contactVidsRepeat.add(vid);
                            a.contactPurchases += 1;
                            a.contactRevenue += rev;
                            if (isRepeat)
                                a.contactRevenueRepeat += rev;
                            else
                                a.contactRevenueNew += rev;
                        }
                        ...
                        if (last) {
                            ...
                            a.lastRevenue += rev;
                            if (isRepeat)
                                a.lastRevenueRepeat += rev;
                            else
                                a.lastRevenueNew += rev;
                        }
    ...
    app.post("/v1/ai/insight", async (req, res) => {
        ...
        try {
            ...
            for (const d of statsSnap.docs) {
                ...
                if (!sid || sid === body.scope_id)
                    continue; // 現在の施策は除外
                const cur = scenarioStatsMap.get(sid) || { imp: 0, clk: 0, cv: 0 };
                if (sd.event === "impression")
                    cur.imp += Number(sd.count || 0);
                else if (sd.event === "click" || sd.event === "click_link")
                    cur.clk += Number(sd.count || 0);
                else if (sd.event === "cv" || sd.event === "conversion")
                    cur.cv += Number(sd.count || 0);
            ...
            for (const d of statsSnap.docs) {
                ...
                if (!path)
                    continue;
                if (sd.event === "pageview")
                    pvMap.set(path, (pvMap.get(path) || 0) + Number(sd.count || 0));
                if (sd.event === "cv" || sd.event === "conversion")
                    cvMap.set(path, (cvMap.get(path) || 0) + Number(sd.count || 0));
            }
    ...
    app.post("/v1/ops/workspaces/options/update", async (req, res) => {
        try {
            ...
            if (typeof rms_enabled === "boolean")
                update.rmsEnabled = rms_enabled;
            if (typeof rms_monthly_price === "number")
                update.rmsMonthlyPrice = rms_monthly_price;
        ...
        try {
            ...
            for (const siteId of siteIds) {
                ...
                if (needAttribution.length === 0)
                    continue;
                // 3. 対象 vid 一覧を取得
                const vids = [...new Set(needAttribution.map((p) => p.vid))];
                // 4. vid ごとに impression ログを取得（Firestore `in` は 10 件上限）
                const vidToImpressions = new Map();
                for (let i = 0; i < vids.length; i += 10) {
                    ...
                    for (const doc of impSnap.docs) {
                        const d = doc.data();
                        if (!d.scenario_id || !d.vid || !d.createdAt)
                            continue;
                        if (!vidToImpressions.has(d.vid))
                            vidToImpressions.set(d.vid, []);
