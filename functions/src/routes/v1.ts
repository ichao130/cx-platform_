...
const LogReqSchema = z.object({
  ...
  uv_first: z.boolean().nullable().optional(),
  session_first: z.boolean().nullable().optional(),
...
const ScenarioTargetingSchema = z.object({
  ...
  audience: z
    .object({
      ...
      weather: z
        .object({
          conditions: z.array(z.enum(["clear", "cloudy", "rain", "snow", "thunder"])).optional().default([]),
          tempBands: z.array(z.enum(["lt10", "t10_19", "t20_24", "t25_29", "ge30"])).optional().default([]),
        })
        .optional()
...
function defaultScenarioTargeting() {
  return {
    ...
    audience: {
      ...
      weather: {
        conditions: [] as Array<"clear" | "cloudy" | "rain" | "snow" | "thunder">,
        tempBands: [] as Array<"lt10" | "t10_19" | "t20_24" | "t25_29" | "ge30">,
      },
    },
    exclude: {
...
function normalizeScenarioTargeting(input: any) {
  ...
  {
    const wCond = ((src as any)?.audience?.weather?.conditions);
    const wBand = ((src as any)?.audience?.weather?.tempBands);
    const COND_OK = ["clear", "cloudy", "rain", "snow", "thunder"];
    const BAND_OK = ["lt10", "t10_19", "t20_24", "t25_29", "ge30"];
    base.audience.weather = {
      conditions: (Array.isArray(wCond) ? wCond : []).map(String).filter((v: string) => COND_OK.includes(v)) as any,
      tempBands: (Array.isArray(wBand) ? wBand : []).map(String).filter((v: string) => BAND_OK.includes(v)) as any,
    };
  }
...
function weatherBucketFromCode(code: number | null): "clear" | "cloudy" | "rain" | "snow" | "thunder" | null {
  if (typeof code !== "number") return null;
  if (code <= 1) return "clear";            // 0快晴 / 1晴れ
  if (code <= 3 || code === 45 || code === 48) return "cloudy"; // 2,3曇り / 45,48霧
  if (code >= 95) return "thunder";         // 95-99 雷雨
  if ((code >= 71 && code <= 77) || (code >= 85 && code <= 86)) return "snow"; // 雪・にわか雪
  if ((code >= 51 && code <= 67) || (code >= 80 && code <= 82)) return "rain"; // 雨・にわか雨
  return null;
}

// 気温(℃) → 帯（幅広め5段階）
function tempBandFrom(temp: number | null): "lt10" | "t10_19" | "t20_24" | "t25_29" | "ge30" | null {
  if (typeof temp !== "number") return null;
  if (temp < 10) return "lt10";
  if (temp < 20) return "t10_19";
  if (temp < 25) return "t20_24";
  if (temp < 30) return "t25_29";
  return "ge30";
}

// シナリオが天気/気温条件を1つでも持つか（＝serve時に気象解決が必要か）
function scenarioNeedsWeather(targeting: any): boolean {
  if (!targeting?.enabled) return false;
...
function matchWeatherTargeting(targeting: any, weather: { code: number | null; temp: number | null }): boolean {
  ...
  if (w.conditions.length > 0) {
    ...
    if (!bucket || !w.conditions.includes(bucket)) return false;
  }
  if (w.tempBands.length > 0) {
    const band = tempBandFrom(weather.temp);
    if (!band || !w.tempBands.includes(band)) return false;
  }
  return true;
}
...
function detectInAppReferrer(ua: string | undefined | null): string | null {
  const s = String(ua || "");
  if (/\bLine\//i.test(s)) return "LINE";              // "online/" 誤検出を防ぐため \b 付き
  ...
  if (/Twitter|TwitterAndroid/i.test(s)) return "X(Twitter)";
...
export function registerV1Routes(app: Express) {
  ...
  app.get("/v1/serve", async (req, res) => {
    try {
      ...
      let serveWeather: { code: number | null; temp: number | null } | null = null;
      if (!isTestMode) {
        const anyNeedsWeather = scenarioSnap.docs.some((d) => scenarioNeedsWeather((d.data() as any)?.targeting));
        if (anyNeedsWeather) {
          try {
            const ip = clientIpFrom(req.header("x-forwarded-for"), req.ip);
            const geo = await resolveGeo(ip);
            let w = (geo.lat != null && geo.lng != null) ? await resolveWeather(geo.lat, geo.lng) : { code: null, temp: null };
            if (w.code == null && w.temp == null) {
              w = await resolveWeather(35.68, 139.76); // 東京フォールバック
            }
            serveWeather = { code: w.code, temp: w.temp };
          } catch (e) {
            try {
              const w = await resolveWeather(35.68, 139.76);
              serveWeather = { code: w.code, temp: w.temp };
            } catch { serveWeather = { code: null, temp: null }; }
          }
        }
      }
      ...
      for (const doc of scenarioSnap.docs) {
        ...
        if (!isTestMode && scenarioNeedsWeather(s?.targeting)) {
          if (!matchWeatherTargeting(s.targeting, serveWeather || { code: null, temp: null })) continue;
        }
      ...
      if (event === "pageview" && body.vid) {
        ...
        if (STATS_LEGACY_DUAL_WRITE) {
          ...
          batch.set(uvRef, {
            ...
            event: "uv", vids: FieldValue.arrayUnion(body.vid), updatedAt: FieldValue.serverTimestamp(),
          }, { merge: true });
          if (body.sid) {
            const sessionRef = db.collection("stats_daily").doc(`${siteId}__${day}__all__all__na__session`);
            batch.set(sessionRef, {
              siteId, day, scenarioId: null, actionId: null, templateId: null, variantId: "na",
              event: "session", sids: FieldValue.arrayUnion(body.sid), updatedAt: FieldValue.serverTimestamp(),
            }, { merge: true });
          }
          if (typeof body.is_new === "boolean") {
            ...
            batch.set(nrRef, {
              siteId, day, scenarioId: null, actionId: null, templateId: null, variantId: "na",
              event: nrEvent, vids: FieldValue.arrayUnion(body.vid), updatedAt: FieldValue.serverTimestamp(),
            }, { merge: true });
          }
        }
        ...
        if (hasFirstFlags) {
          const bumpCounter = (ev: string) => {
            const ref = db.collection("stats_daily").doc(
              `${siteId}__${day}__all__all__na__${ev}__s${pickStatShard()}`
            );
            batch.set(ref, {
              siteId, day, scenarioId: null, actionId: null, templateId: null, variantId: "na",
              event: ev, count: FieldValue.increment(1), updatedAt: FieldValue.serverTimestamp(),
            }, { merge: true });
          };
          if (body.uv_first) bumpCounter("uv");
          if (body.sid && body.session_first) bumpCounter("session");
    ...
    try {
      ...
      if (body.scope === "scenario") {
        try {
          ...
          isPurchaseGoal = scSnap.exists && (scSnap.data() as any)?.goal?.type === "purchase";
      ...
      for (const doc of snap.docs) {
        ...
        else if (ev === "conversion" && !isPurchaseGoal) bucket.conversions += c;
        else if (ev === "purchase" && isPurchaseGoal) bucket.conversions += c;
      }
  ...
  app.post("/v1/questions/answers/summary", async (req, res) => {
    ...
    try {
      ...
      const db = adminDb();
      const byKey: Record<string, Record<string, number>> = {}; // key -> value -> ユーザー数
      const answeredCounts: Record<string, number> = {};        // questionId -> 回答者数
      let totalWithAttrs = 0;
      let scanned = 0;
      let truncated = false;
      const MAX_SCAN = 50000;

      let cursor: any = null; // last document snapshot
      while (true) {
        ...
        for (const d of snap.docs) {
          ...
          const answered = (data.answered || {}) as Record<string, string>;
          let hasAny = false;
      ...
      const out = await callOpenAIJson({
        ...
        schema: z.object({
          segment_suggestions: z.array(z.object({
            segment_label: z.string().optional().default(""),
            attribute_key: z.string().optional().default(""),
            attribute_value: z.string().optional().default(""),
            why: z.string().optional().default(""),
            message_idea: z.string().optional().default(""),
          })).optional().default([]),
  ...
  app.post("/v1/push/subscribe", async (req, res) => {
    try {
      ...
      if (!site_id || !subscription) return res.status(400).json({ ok: false, error: "missing_params" });

      // site_key でサイトの正当性を確認
      const db = adminDb();
      const siteDoc = await db.collection("sites").doc(site_id).get();
      if (!siteDoc.exists) return res.status(404).json({ ok: false, error: "site_not_found" });
      ...
      let subObj: any;
      try { subObj = typeof subscription === "string" ? JSON.parse(subscription) : subscription; } catch { return res.status(400).json({ ok: false, error: "invalid_subscription" }); }
  ...
  app.post("/v1/push/subscribers/count", async (req, res) => {
    try {
      ...
      if (!site_id) return res.status(400).json({ ok: false, error: "missing_site_id" });

      const db = adminDb();
      const siteDoc = await db.collection("sites").doc(site_id).get();
      if (!siteDoc.exists) return res.status(404).json({ ok: false, error: "site_not_found" });
      const siteData = siteDoc.data() || {};
      await assertWorkspaceRole({ workspaceId: siteData.workspaceId || "", uid, allowedRoles: ["owner", "admin", "member", "viewer"] });
