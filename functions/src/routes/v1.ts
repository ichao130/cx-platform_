...
const LogReqSchema = z.object({
  ...
  event: z.enum(["impression", "click", "click_link", "close", "conversion", "pageview", "pageleave"]),
...
const WorkspaceBillingUpdateReqSchema = z.object({
  ...
  provider: z.enum(["stripe", "misoca", "manual"]).optional(),
...
const PlansUpsertReqSchema = z.object({
  ...
  code: z.enum(["free", "standard", "pro", "enterprise"]),
  name: z.string().min(1).max(80),
  description: z.string().max(2000).optional().default(""),
  active: z.boolean().optional().default(true),
  billing_provider: z.enum(["stripe", "misoca", "manual"]),
...
function normalizeBillingProvider(input: any, plan?: string) {
  ...
  if (raw === "stripe" || raw === "misoca" || raw === "manual") return raw;
...
export function registerV1Routes(app: Express) {
  ...
  app.post("/v1/workspaces/access-override/set", async (req, res) => {
    try {
      ...
      const body = z.object({
        workspace_id: z.string().min(1),
        days: z.number().int().min(0).max(3650).optional(), // 0で解除
      ...
      return res.status(
        ...
        : 400
      ).json({ ok: false, error: "access_override_failed", message: e?.message || String(e) });
    }
  });
  ...
  app.post("/v1/stripe/create-checkout-session", async (req, res) => {
    try {
      ...
      const existingCustomerId = (billingSnap.data() as any)?.stripe_customer_id || undefined;
      ...
      return res.status(
        ...
        : 400
      ).json({ ok: false, error: "stripe_checkout_failed", message: e?.message || String(e) });
    }
  });
  ...
  app.post("/v1/stripe/create-portal-session", async (req, res) => {
    try {
      ...
      const db = adminDb();
      const billingSnap = await db.collection("workspace_billing").doc(body.workspace_id).get();
      const customerId = (billingSnap.data() as any)?.stripe_customer_id;
      ...
      return res.status(
        ...
        : 400
      ).json({ ok: false, error: "stripe_portal_failed", message: e?.message || String(e) });
    }
  });
  ...
  app.post("/v1/stripe/webhook", async (req, res) => {
    try {
      ...
      if (!sig) return res.status(400).json({ ok: false, error: "missing_stripe_signature" });
  ...
  app.get("/v1/misoca/callback", async (req, res) => {
    try {
      ...
      if (!workspaceId) return res.status(400).send("missing workspace_id in state");
  ...
  app.post("/v1/misoca/create-invoice", async (req, res) => {
    try {
      ...
      const body = z.object({
        workspace_id: z.string().min(1),
        billing_email: z.string().email().optional(),
        issue_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional(), // "2026-04-01"
        due_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional(),
        items: z.array(z.object({
          name: z.string().min(1),
          unit_price: z.number().min(0),
          quantity: z.number().int().min(1).default(1),
          tax_type: z.string().optional().default("tax_8"),
        })).min(1),
      ...
      if (!tokenSnap.exists) throw new Error("misoca_not_connected");
      ...
      return res.status(
        ...
        : 400
      ).json({ ok: false, error: "misoca_create_invoice_failed", message: e?.message || String(e) });
    }
  });
