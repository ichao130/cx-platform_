...
async function getMisocaTokenData(): Promise<any | null> {
  const db = adminDb();
...
export async function getMisocaAccessToken(clientId: string, clientSecret: string): Promise<string> {
  const tokenData = await getMisocaTokenData();
  if (!tokenData) throw new Error("MISOCA が未連携です。バックヤードから連携してください。");
  ...
  const newRefreshToken = (json.refresh_token || tokenData.refresh_token) as string;
  const newExpiresAt = new Date(now.getTime() + (json.expires_in || 7200) * 1000).toISOString();

  await adminDb().collection(SYSTEM_CONFIG_DOC).doc(MISOCA_DOC).set({
...
/** MISOCA 接続状態を取得 */
export async function getMisocaStatus(): Promise<MisocaStatus> {
...
async function createAndSendInvoice(
  ...
  if (!invoiceId) throw new Error("MISOCA から invoiceId が取得できませんでした");
...
export async function sendMisocaInvoicesJob(
  clientId: string,
  clientSecret: string
): Promise<{
  success: number;
  ...
  for (const billingDoc of wsBillingSnap.docs) {
    ...
    const wsBilling = billingDoc.data() as any;
